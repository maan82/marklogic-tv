...
declare function assert:not-equal($actual as item()*, $expected as item()*) as xs:string? {
  assert:not-equal($actual, $expected, "")
};

declare function assert:not-equal($actual as item()*, $expected as item()*, $msg as xs:string?) as xs:string? {
...
declare function assert:not-empty($val as item()*) as xs:string? {
  assert:not-empty($val, "")
};

declare function assert:not-empty($val as item()*, $msg as xs:string?) as xs:string? {
  if (fn:not(fn:empty($val))) then
    ()
  else
    fn:concat($msg, " - expected: (), actual: ", $val[1], "...")
};
