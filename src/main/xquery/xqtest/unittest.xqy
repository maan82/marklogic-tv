...
declare function unittest:findTestModules($testRoot as xs:string?) as xs:string* {
  if (xdmp:modules-database() eq 0) then
    ...
    iv:invoke("ml-tv-xquery", xdmp:function(xs:QName("unittest:findModulesInModulesDb")), $testRoot)
};
    ...
    return element module {
      ...
      return (
        ...
        return if($i > 1) then
          ...
          (),
        for $skipped at $i in fn:tokenize($text, '\(:SKIP:\)')
        return
          if ($i > 1) then
            element skip {
              attribute name { fn:normalize-space(fn:substring-before($skipped, '(')) }
            }
          else
            ()
      )
...
declare function unittest:runInternal($modules as element(modules)) {
  ...
  let $trace := xdmp:trace("xqtest", xdmp:quote($suite))
